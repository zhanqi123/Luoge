  ...
  <div id="app" class="registration-container">
    <el-card class="box-card">
      <div class="custom-form">
        <h3 class="form-title">项目过程文件审核</h3>
        ...
        <!-- 审核意见区域 -->
        <!-- <el-descriptions title="审核意见" :column="1" border class="audit-opinion" v-if="formFlag">
        ...
        <div class="submit-buttons">
          <el-button type="success" v-if="this.formFlag" icon="el-icon-folder-checked" class="submit-btn"
            :loading="submitting" @click="debouncedSubmit">
            提交审核
          </el-button>
          <el-button v-else type="danger" icon="el-icon-circle-close" class="submit-btn" :loading="submitting"
    ...
    <el-dialog title="请输入驳回原因" :visible.sync="rejectDialogVisible" width="40%" center>
      <el-input type="textarea" v-model="rejectReason" :rows="4" placeholder="请输入驳回原因"></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="rejectDialogVisible = false">取 消</el-button>
        <el-button type="warning" @click="confirmReject">确 定</el-button>
      </span>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      rejectReason: '', // 已有字段，确认是否存在


      url: 'http://192.168.16.26:81/auditoneRegister?ReportDate=2025-04-01&SystemID=LG-SYS-00425&uuid=token%3ATUpcPHcqvAlGsx3L0Pn%20aHH1jHb7%203CGw%2FmkQjz32hGW9dSJbeR4JWieFu3I%2FoWkSoW%20Qdqo6yVB0oyKhp0hwyiMDUGURYzQ'

    }
  },
  created() {
  // 创建防抖函数
    this.debouncedSubmit = debounce(this.handleSubmit, 500);
    this.parseQueryData();
    this.initFromflag();
    this.getList();
  ...
  methods: {
    ...
    initFromflag() {
      ...
      initList(data).then(res => {
        ...
        if (res.Data) {
          ...
          this.formFlag = res.Data[0].一阶段审核状态 == '一阶段审核通过' ? false : true
          setTimeout(() => {
    ...
    async confirmReject() {
      ...
      try {
        ...
        if (res.Data) {
          this.$modal.msgSuccess('该表单已被驳回~');
          this.initFromflag();
        } else {
          this.$message.error(res.ErrorInfo || '驳回失败');
        }
      } catch (error) {
    ...
    async handleSubmit() {
      try {
        ...
        const confirmResult = await this.$confirm('请确认该过程文件审核通过?', '提示', {
        ...
        const res = await submitList(submitData);
        if (res.Data) {
          this.$message({
            message: '审核提交成功',
            type: 'success',
            ...
            onClose: async () => {
              try {
                await this.initFromflag();
                // this.$router.go(-1);
              } catch (error) {
                console.error('初始化表单状态失败:', error);
                this.$message.error('初始化表单状态失败，请稍后重试');
              }
            }
          });
        } else {
    ...
    notGo() {
      this.$confirm('请确认你要驳回该审核通过状态, 是否继续?', '警告！', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
...
.el-dialog__body .el-textarea__inner:focus {
  /* border-color: #f56c6c; */
